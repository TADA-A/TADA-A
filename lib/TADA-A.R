...
TADA_A_RR_estimate_for_single_study <-function(data, feature_number = 1, gene_prior_file){
  ...
  if (feature_number == 1){
    mle <- optim(rep(0.1, feature_number), fr,method = "Brent", lower = -1, upper = 10,control=list("fnscale"=-1, "maxit" = 2000), hessian = TRUE)
  }
  else{
    mle <- optim(rep(0.1, feature_number), fr,control=list("fnscale"=-1, "maxit" = 2000), hessian = TRUE)
  }
  list(mle = mle)
}
...
TADA_A_reading_in_annotations <- function(mut_file = "../data/Yuen_NM2015_cases_DNM.bed",
  ...
  # [estimate_RR] Whether RR estimation is needed for this single study.
...
TADA_A_RR_estimate_from_multiple_studies <-function(data_list, selected_annotations, gene_prior_file){
  ...
  if (feature_number == 1){
    mle <- optim(rep(0.1, feature_number), fr, method = "Brent", lower = -1, upper = 5,control=list("fnscale"=-1, "maxit" = 2000), hessian = TRUE)
  }else{
    mle <- optim(rep(0.1, feature_number), fr ,control=list("fnscale"=-1, "maxit" = 2000), hessian = TRUE)
  }
...
TADA_A_RR_estimate_for_single_study_AS <-function(data, gene_prior_file){
  #[data] is the object in the form of [AS_info] generated by [TADA_A_read_in_AS_annotation], which has compact base-level information. 
  #[gene_prior_file], a file that has the prior probability of each gene as a risk gene. e.g., "../data/Example_gene_prior.txt"
  ...
  mle <- optim(0.1, fr, method = "Brent", lower = -1, upper = 10, control=list("fnscale"=-1), hessian = TRUE)
  list(mle = mle)
}


# read in allele-specific annotation
TADA_A_read_in_AS_annotation <- function(mut_file = "../data/Yuen_NM2015_cases_DNM_CADD_gt10.bed", # the bed file with mutations whose allele specific deleterious scores pass a certain threshold
                                         window_file = "../data/Example_windows.bed",
                                         #mutrate_file = "../data/Example_windows.mutrate",
                                         mutrate_scaling_file = "../data/Example_windows_mutrate_scaling_file_for_Yuen_NM2015_cases_DNM.txt",
                                         sample_size = 162,
  ...
  # this function could either only output data partitioned in the compact form for later combined RR estimation with other dataset, or do RR estimation using data from this single study
  if(estimate_RR){
    RR <- TADA_A_RR_estimate_for_single_study_AS(data = AS_data_partition, gene_prior_file = gene_prior_file)
    return(list(AS_info = AS_data_partition, mle = RR$mle))
  }else{
    return(list(AS_info = AS_data_partition, mle = NA))
  }
}


#estimate RR from multiple studies 
TADA_A_RR_estimate_from_multiple_studies_AS <-function(data_list, gene_prior_file){
  #[data_list] is a list of objects [AS_info] returned from [TADA_A_reading_in_annotations_AS], each element of the list has compact base-level information for each gene. 
  #[gene_prior_file], #[gene_prior_file], a file that has the prior probability of each gene as a risk gene. e.g., "../data/Example_gene_prior.txt".
  ...
  mle <- optim(0.1, fr, method = "Brent", lower = -5, upper = 10, control=list("fnscale"=-1), hessian = TRUE)
  list(mle = mle)
}



TADA_A_RR_estimate_from_multiple_studies_170820_temp <-function(data_list, selected_annotations, gene_prior_file){
  #[data_list] is a list of objects [base_info] returned from [TADA_A_reading_in_annotations], each element of the list has compact base-level information for each gene. 
  #[selected_annotations] is a vector indicating non-coding annotations whose RRs need to be estimated. e.g., c(2,3) means that the 2nd and 3rd annotations in the [noncoding_annotations] argument of [TADA_A_reading_in_annotations] will have their RRs estimated.
  #[gene_prior_file], #[gene_prior_file], a file that has the prior probability of each gene as a risk gene. e.g., "../data/Example_gene_prior.txt".
...
TADA_A_RR_estimate_from_multiple_studies_temp2 <-function(data_list, selected_annotations, gene_prior_file){
  #[data_list] is a list of objects [base_info] returned from [TADA_A_reading_in_annotations], each element of the list has compact base-level information for each gene. 
  #[selected_annotations] is a vector indicating non-coding annotations whose RRs need to be estimated. e.g., c(2,3) means that the 2nd and 3rd annotations in the [noncoding_annotations] argument of [TADA_A_reading_in_annotations] will have their RRs estimated.
  #[gene_prior_file], #[gene_prior_file], a file that has the prior probability of each gene as a risk gene. e.g., "../data/Example_gene_prior.txt".
